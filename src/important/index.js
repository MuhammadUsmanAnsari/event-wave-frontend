...
window.categories = ["Business", "Concert", "Sports", "Party", "Wedding", "Birthday", "Seminar", "Festival", "Travel", "Other"];

window.verifyImageSize = (file) => {
    const maxSize = 2 * 1024 * 1024; // 5MB in bytes

    if (file) {
        if (file.type === 'image/png' || file.type === 'image/jpeg') {
            if (file.size <= maxSize) {
                // File is valid, you can proceed with further actions (e.g., upload)
                return file
            } else {
                window.toastify('File size exceeds 2MB limit.', "error");
            }
        } else {
